...
?>
<?php
class MagenThemes_MTBejewelry_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        return Mage::getStoreConfig('mtbejewelry/' . $optionString);
    }
    ...
    {
        if ($storeId)
            return Mage::getStoreConfig('mtbejewelry_design', $storeId);
        else
            return Mage::getStoreConfig('mtbejewelry_design');
    }
    ...
    {
        return Mage::getStoreConfig('mtbejewelry_design/' . $optionString, $storeCode);
    }
    ...
    {
        return Mage::getStoreConfig('mtbejewelry_layout/' . $optionString, $storeCode);
    }
    ...
    public function getAltImgHtml($product, $w, $h, $imgVersion='small_image')
    {
        ...
        if ($gal = $product->getMediaGalleryImages())
        {
            ...
            {
                return
                    '<img class="img-responsive alt-img lazy" data-src="' . Mage::helper('mtbejewelry/image')->getImg($product, $w, $h, $imgVersion, $altImg->getFile()) . '" alt="' . $product->getName() . '" />';
            }
        }
    ...
    {
        $config['zoomWidth'] = is_numeric(Mage::getStoreConfig('mtbejewelry/product_page/zoom_width'))?Mage::getStoreConfig('mtbejewelry/product_page/zoom_width'):'auto';
        $config['zoomHeight'] = is_numeric(Mage::getStoreConfig('mtbejewelry/product_page/zoom_height'))?Mage::getStoreConfig('mtbejewelry/product_page/zoom_height'):'auto';
        $config['position'] = Mage::getStoreConfig('mtbejewelry/product_page/zoom_position')?Mage::getStoreConfig('mtbejewelry/product_page/zoom_position'):'right';
        $config['adjustX'] = is_numeric(Mage::getStoreConfig('mtbejewelry/product_page/zoom_adjustX'))?Mage::getStoreConfig('mtbejewelry/product_page/zoom_adjustX'):0;
        $config['adjustY'] = is_numeric(Mage::getStoreConfig('mtbejewelry/product_page/zoom_adjustY'))?Mage::getStoreConfig('mtbejewelry/product_page/zoom_adjustY'):0;
        if ($rel){
            $out = array();
