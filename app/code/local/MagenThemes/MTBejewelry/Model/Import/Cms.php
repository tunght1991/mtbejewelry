...
class MagenThemes_MTBejewelry_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
        ...
        try
        {
            ...
            foreach ($xmlObj->getNode($itemContainerNodeString)->children() as $item)
            {
                ...
                foreach($item->page as $object){
                    Mage::getSingleton('mtbejewelry/resource_widget')->importInstancePage($model->getInstanceId(), $object, $item->sort_order, $item->xml);
                }
                $i++;
            }
            //Final info
