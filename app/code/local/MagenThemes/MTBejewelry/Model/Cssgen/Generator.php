...
?>
<?php
class MagenThemes_MTBejewelry_Model_Cssgen_Generator extends Mage_Core_Model_Abstract{
    ...
    protected function _generateStoreCss($design, $storeCode){
        ...
        $filedefault = Mage::helper('mtbejewelry/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'magenthemes/mtbejewelry/css/' . $design . '.phtml';
        ...
        try{
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('mtbejewelry')->__("Template file is empty or doesn't exist: %s", $path) );
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array( 'path' => Mage::helper('mtbejewelry/cssgen')->getGeneratedCssDir() ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mtbejewelry')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('mtbejewelry/cssgen')->getGeneratedCssDir()). '<br/>Message: ' . $gener->getMessage());
